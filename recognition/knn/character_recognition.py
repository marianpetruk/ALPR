...
def recognize_characters(possible_plate):
    ...
    list_of_possible_chars_in_plate = findPossibleCharsInPlate(plate_gray, plate_thresh)
    listOfListsOfMatchingCharsInPlate = findListOfListsOfMatchingChars(list_of_possible_chars_in_plate)
    if (len(listOfListsOfMatchingCharsInPlate) == 0):
        return ""

    for i in range(len(listOfListsOfMatchingCharsInPlate)):  # within each list of matching chars
        ...
        return recognizeCharsInPlate(plate_thresh, longestListOfMatchingCharsInPlate)
...
def recognizeCharsInPlate(imgThresh, listOfMatchingChars):
    ...
    for currentChar in listOfMatchingChars:  # for each char in plate
        ...
        retval, npaResults, neigh_resp, dists = k_nearest.findNearest(npaROIResized,
...
def loadKNNDataAndTrainKNN(classifictation_file, flat_images_file):
    ...
    k_nearest.setDefaultK(1)  # set default K to 1

    k_nearest.train(npaFlattenedImages, cv2.ml.ROW_SAMPLE, npaClassifications)  # train KNN object
...
if __name__ == "__main__":
    ...
    chars = recognize_characters(image)
    if chars:
        print(chars)
    else:
